...
async function main() {
  ...
  // Custom mappings for country names that i18n-iso-countries might fail to parse perfectly
  ...
  const missingInDb = []
  const wrongGroupInDb = []
  let correctInDb = 0

  for (const target of targetMapping) {
    const dbItem = dbMap.get(target.code)
    if (!dbItem) {
      missingInDb.push(target)
    } else {
      if (dbItem.group !== target.targetGroup) {
        wrongGroupInDb.push({ ...target, currentGroup: dbItem.group })
      } else {
        correctInDb++
      }
    }
  ...
  console.log("=== RÀ SOÁT DANH SÁCH QUỐC GIA ===")
  console.log(`- Tổng số quốc gia trên MD: ${targetMapping.length + unresolved.length}`)
  console.log(`- Không nhận diện được code: ${unresolved.length}`)
  if (unresolved.length > 0) {
    console.log(unresolved)
  }
  
  console.log(`\n- Số lượng đã khớp chuẩn (trong DB có và group đúng): ${correctInDb}`)
  
  console.log(`\n- Số lượng CÓ trong DB nhưng SAI Group: ${wrongGroupInDb.length}`)
  if (wrongGroupInDb.length > 0) {
    console.table(wrongGroupInDb)
  }

  console.log(`\n- Số lượng THIẾU trong DB (Có trong file MD nhưng chưa có ở bảng Nationality): ${missingInDb.length}`)
