...
> ⚠️ **Admin project note:** This rule applies only when the admin dashboard accesses the database **directly** via Prisma Server Components (e.g. internal admin queries). For user-facing operations, always go through the `api/` ExpressJS service via HTTP — do NOT bypass the API layer for business logic.
