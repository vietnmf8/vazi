...
<nextjs-turbopack-dev>
<rules>
- KHÔNG bật `turbopackFileSystemCacheForDev: true` — tích lũy cache không giới hạn vào .next/dev/, load toàn bộ vào RAM mỗi lần restart → gây memory leak theo thời gian
- KHÔNG bật `cacheComponents: true` trừ khi codebase đang dùng `use cache` directive — experimental flag này thêm overhead mà không có lợi ích gì nếu không có `use cache`
- Nếu RAM dev server tăng chóng mặt theo thời gian: kiểm tra kích thước `.next/dev/cache/`, xóa `.next/` và restart
- Dùng `LazyMotion` + `m.*` thay `motion.*` từ framer-motion để tránh chạy animation engine trên server
- Dynamic import các thư viện nặng (emoji-picker-react, heavy chart libs) chỉ load khi cần, không import static
- `new Audio()` phải cleanup sau khi phát: `audio.addEventListener("ended", () => { audio.src = "" }, { once: true })`
- `addEventListener` scroll/resize phải có `{ passive: true }` để không block main thread
</rules>

<diagnosis>
Khi thấy RAM Node.js tăng liên tục theo thời gian trong dev:
1. Chạy `Get-Process node | Select Id, @{N='RAM(MB)';E={[math]::Round($_.WorkingSet64/1MB,0)}}` để xác định process nặng nhất
2. Kiểm tra `.next/dev/cache/` — nếu > 500MB là dấu hiệu `turbopackFileSystemCacheForDev` đã tích lũy
3. Xóa `.next/` và tắt `turbopackFileSystemCacheForDev`

Khi thấy `application-code` trong Next.js dev log cao bất thường:
- `next.js: 2ms, application-code: 500ms` → Turbopack đã cache compile rồi, vấn đề là SSR render cost
- Kiểm tra các `"use client"` component có import `motion.*` không → đổi sang `m.*` + `LazyMotion`
- `reactStrictMode: true` gây double-render trên server trong dev → application-code nhân đôi so với production
</diagnosis>

<examples>
<example type="bad">
```ts
// next.config.ts — CẤM dùng các flag này khi không cần
const nextConfig = {
  cacheComponents: true,           // ❌ không có use cache nào
  experimental: {
    turbopackFileSystemCacheForDev: true,  // ❌ cache tích lũy không giới hạn
  },
}
```
</example>
...
<example type="good">
```ts
// next.config.ts — config sạch, không experimental không cần thiết
