...
## LazyMotion — Bắt buộc trong Next.js App Router

**LUÔN dùng `LazyMotion` + `m.*` thay cho `motion.*`.** Nếu dùng `motion.*`, framer-motion chạy toàn bộ animation engine trên server mỗi SSR request → tăng `application-code` time đáng kể.
...
```tsx
// src/app/layout.tsx — bọc toàn bộ app
...
```tsx
// ❌ SAI — animation engine chạy trên server
import { motion } from "framer-motion"
<motion.div animate={{ opacity: 1 }}>...</motion.div>

// ✅ ĐÚNG — animation engine chỉ load trên client sau hydration
import { m } from "framer-motion"
<m.div animate={{ opacity: 1 }}>...</m.div>

// AnimatePresence import giữ nguyên — không đổi
import { AnimatePresence, m } from "framer-motion"
```

### domAnimation vs domMax

| Feature | domAnimation (~18kb) | domMax (~33kb) |
|---|---|---|
...
Dùng `domMax` nếu component có `drag` hoặc `layout` prop. Dùng `domAnimation` nếu chỉ cần animate/transition cơ bản.
