...
```ts
// ⚠️ `cacheComponents: true` (Next.js 16)
// Trong môi trường Dev (npm run dev), flag này ép Next.js phải tracking I/O
// cho Partial Prerendering (PPR), khiến `application-code` chậm thêm 600-1200ms
// mỗi lần tải trang, kể cả khi dùng mock data.
// TRONG PRODUCTION (npm run build && npm start): Cực nhanh, TTFB ~ 5-50ms.
// Fix: Nếu thấy dev quá chậm, có thể comment tạm thời lúc code UI.
...
| `application-code` > 400ms cho trang mock | `cacheComponents: true` tracking overhead | Comment flag khi dev UI, bật lại khi test data/build |
