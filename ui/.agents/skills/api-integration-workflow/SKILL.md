...
## Pitfalls — Form đã có sẵn (Stage 6 → Stage 7)

Khi ghép API vào `/apply` (calculate-price, submit, PayPal):

| Pitfall | Cách tránh |
| -------- | ------------ |
| `watch()` + `setState` parent trong auto-save | Chỉ persist draft/localStorage khi gõ; state step cập nhật khi `onNext` |
| Gọi API calculate mỗi keystroke | Debounce 300–500ms hoặc chỉ khi đổi field ảnh hưởng giá |
| Reset form khi API trả về | `reset(data)` có chủ đích, không bind `defaultValues` từ parent mỗi render |
| Upload mock filename → API | Stage 7: presigned URL trước, lưu URL vào form state |

Rule chi tiết: `ui/.cursor/rules/25-multi-step-apply-forms.mdc`.
