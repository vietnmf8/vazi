...
description: Next.js 16+ Cache Components — PPR, use cache directive, cacheLife, cacheTag, updateTag. ⚠️ Only use if Next.js >= 16 is confirmed in package.json.
---

# Cache Components (Next.js 16+)

> ⚠️ **Version check required:** This feature requires **Next.js 16+**. Before applying, verify `package.json` — if `"next"` is `< 16`, these APIs do not exist yet.
> Run: `node -e "const p=require('./package.json'); console.log(p.dependencies?.next || p.devDependencies?.next)"`
...
## FASTVISA pitfalls (`cacheComponents: true`)

| Lỗi | Nguyên nhân | Fix |
| --- | --- | --- |
| `new Date()` in Client Component without Suspense | Copyright/footer year trong `"use client"` không bọc Suspense | `Suspense` ở layout/footer **hoặc** async SC + `connection()` |
| `new Date()` before uncached data | `new Date()` trực tiếp trong SC khi prerender | `await connection()` trước `new Date()` |
| Uncached data outside `<Suspense>` | SC async `connection()` render trong static shell | Bọc component trong `<Suspense fallback={...}>` |
| `React.Children.only` với `<Button asChild>` | Spinner `null` + `children` = 2 children cho Radix Slot | Nhánh `asChild`: chỉ `{children}` — xem `06-shadcn-semantic.mdc` |

**Verify bắt buộc sau khi sửa layout/footer/button:**
```bash
