...
### Gotcha — `asChild` + Radix `Slot` = đúng **một** React element

`@radix-ui/react-slot` gọi `React.Children.only()` — **không** được có sibling (kể cả `null`).

**❌ SAI — Spinner/fragment cạnh `children` khi `asChild` (FASTVISA Stage 6 — Homepage 500):**
```tsx
function Button({ asChild, isLoading, children, ...props }) {
  const Comp = asChild ? Slot : "button"
  return (
    <Comp {...props}>
      {isLoading ? <span className="animate-spin" /> : null}  {/* ← sibling */}
      {children}  {/* ← Link — Children.only throws */}
    </Comp>
  )
}
```

**✅ ĐÚNG — Tách nhánh `asChild`: chỉ pass `children` vào `Slot`:**
```tsx
if (asChild) {
  return <Slot className={...} {...props}>{children}</Slot>
}
return (
  <button {...props}>
...
**Triệu chứng:** `React.Children.only expected to receive a single React element child` trên `/` khi nhiều `<Button asChild><Link /></Button>`.
