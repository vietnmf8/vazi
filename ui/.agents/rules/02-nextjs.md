...
### Route group — layout khác nhau, URL không đổi

Dùng khi một route cần chrome khác (VD: `/apply` không Footer):

```
app/
├── layout.tsx           # html/body only
├── (main)/layout.tsx    # Header + Footer → /, /faqs, ...
└── apply/layout.tsx     # Header only → /apply
```

**Không** đặt Header/Footer ở root rồi ẩn bằng CSS theo pathname.
...
**❌ SAI — Client Component `new Date()` KHÔNG bọc Suspense (build/prerender fail):**
```tsx
// CopyrightYear.tsx
"use client"
...
// Footer.tsx — thiếu <Suspense>
<footer>© <CopyrightYear /></footer>
```
→ `used new Date() inside a Client Component without a Suspense boundary`

**✅ ĐÚNG — Pattern FASTVISA (đã verify `npm run build` pass):**

```tsx
// CopyrightYear.tsx — async Server Component
import { connection } from "next/server"

export async function CopyrightYear() {
  await connection()  // đánh dấu dynamic trước new Date()
  return new Date().getFullYear()
}

// Footer.tsx — BẮT BUỘC Suspense quanh component gọi connection()
import { Suspense } from "react"
...
export function Footer() {
  return (
    <footer>
      ©{" "}
      <Suspense fallback={<span aria-hidden="true">2026</span>}>
        <CopyrightYear />
      </Suspense>{" "}
      VietnamEVisa
    </footer>
...
**Hoặc** Client Component + Suspense ở parent (fallback năm tĩnh), nếu không dùng `connection()`.
...
→ Defer bằng `connection()` / `cookies()` / `headers()` **trước**, hoặc Client + `<Suspense>`, hoặc `'use cache: private'`.
